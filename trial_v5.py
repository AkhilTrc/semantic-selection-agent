...
def run_empowerment_loop(
    ...
    validity_track = []  # track last three validity statuses
    for iteration in range(1, max_iters+1):
        ...
        for _ in range(LLM_MAX_RETRIES):
            # Adjust temperature: look at last three valid/invalid statuses
            if len(validity_track) >= 3:
                last_three = validity_track[-3:]
                if all(not status for status in last_three):  # all invalid
                    temperature = max(TEMPERATURE_MIN, temperature - 0.1)
                elif all(status for status in last_three):  # all valid
                    temperature = min(TEMPERATURE_MAX, temperature + 0.1)
                # else: if a mix of valid/invalid, do nothing
            ...
            break
        if chosen is None:
            validity_track.append(False)  # failed = invalid
            history.append({
                "iter": iteration,
                "event": "no_llm_choice",
